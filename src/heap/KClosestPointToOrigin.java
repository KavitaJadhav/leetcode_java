...
//| Metric | Value                                   |
//| ------ | --------------------------------------- |
//| Time   | **O(N log k)** (heap insertions)        |
//| Space  | **O(k)** (heap stores at most k points) |
//Always use Integer.compare() in Java instead of b-a to avoid integer overflow.
//
//Max heap of size k is the canonical solution for K closest points problems.
//
//For a faster O(N) solution, QuickSelect can be used, but heap is safer for coding interviews.
