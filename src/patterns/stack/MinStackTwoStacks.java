...
//| --------------- | ---------- | ------------------------------------------------------------------------------------- |
//| `push(int val)` | **O(1)**   | Only two `push()` operations (patterns.stack + possibly minStack). Comparing `peek()` is O(1). |
//| `pop()`         | **O(1)**   | Only two `pop()` operations at most. `peek()` and `Objects.equals()` are O(1).        |
//| `top()`         | **O(1)**   | Just `patterns.stack.peek()`.                                                                  |
//| `getMin()`      | **O(1)**   | Just `minStack.peek()`.                                                               |

//2️⃣ Space Complexity
//Let n = number of elements in the patterns.stack.
//patterns.stack stores all elements → O(n)
