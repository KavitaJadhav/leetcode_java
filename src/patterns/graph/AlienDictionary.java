...
//DFS recursion patterns.stack → O(C)
...
//Build patterns.graph	O(N × L)	O(C + E) ≈ O(C²)
//DFS traversal	O(C + E) ≈ O(C²)	O(C) recursion patterns.stack
//Total	O(N × L + C²)	O(C²)
    ...
    private Map<Character, List<Character>> buildDependencyMap(String[] words) {
        ...
        for (int word_index = 0; word_index < words.length - 1; word_index++) {
            ...
            for (int char_index = 0; char_index < min_length; char_index++) {
                ...
                if (char1 != char2) {
                    dependencyMap.get(char2).add(char1);
                    break;
                }
            }
        }
